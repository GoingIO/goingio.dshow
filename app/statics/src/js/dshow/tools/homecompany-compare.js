...
//		info.push('</div>');
        ...
        app.controller("appCtrl", ["$scope", "applyService", "$location", function ($scope, applyService, $location) {
            ...
            function markHomeCompany(data, userId_md5, userIdOrLngLat, isCompare) {
                    ...
                    if (!diggedHome(attr) && !diggedCompany(attr)) {
                        if (diggedPermanent(attr)) {
                            $scope.showInfoWindowAndMarker({
                                ...
                                if (isCompare) {
                                    $scope.permanentDigCompare = data
                                } else {
                        ...
                        if (diggedHome(attr)) {
                            $scope.showInfoWindowAndMarker({
                                x: attr.u_home_x,
                                y: attr.u_home_y,
                                image: DSHOW_CONTEXT + '/statics/images/custom_a_b.png',
                                imageOffset: isCompare ? new AMap.Pixel(-38, 0) : new AMap.Pixel(0, 0) ,
                                windowTitle: isCompare ? '家的位置[对比]' : '家的位置',
                                zIndex: isCompare ? 99 : 100,
                                windowData: {
                            ...
                            if (isCompare) {
                                homeTemp2 = [attr.u_home_x, attr.u_home_y];
                            } else {
                                homeTemp1 = [attr.u_home_x, attr.u_home_y];
                            }

                        } else {
                            $scope.homeDig = $scope.homeDigCompare = $scope.nullHint
                            applyService.$apply()
                        }
                        ...
                        if (diggedCompany(attr)) {
                            $scope.showInfoWindowAndMarker({
                                ...
                                imageOffset: isCompare ? new AMap.Pixel(-114, 0) : new AMap.Pixel(-76, 0),
                                windowTitle: isCompare ? '公司的位置[对比]' : '公司的位置',
                                zIndex: isCompare ? 99 : 100,
                                windowData: {
                            ...
                            if (isCompare) {
                                companyTemp2 = [attr.u_company_x, attr.u_company_y];
                            } else {
                                companyTemp1 = [attr.u_company_x, attr.u_company_y];
                            }

                        } else {
                            $scope.companyDig = $scope.companyDigCompare = $scope.nullHint
                            applyService.$apply()
                        }
            ...
            $scope.showInfoWindowAndMarker = function (options) {
                ...
                getAddressPromise.then(function (address) {
                    var marker = new AMap.Marker({
                        ...
                        icon: new AMap.Icon({
                            size: options.size || new AMap.Size(38, 32),
                            image: options.image,
            ...
            $scope.search = function () {
                ...
                var a2 = dataloader.get(userId_md5, datasource.getSource('dshow_home_company_ab'), {
                    source: 'home',
                    type: 'post',
                    disableErrCallback: true
                });
                var a1 = dataloader.get(userId_md5, datasource.getSource('dshow_home_company'), {
                    source: 'home',
                    type: 'post',
                    disableErrCallback: true
                });

                $.when(a1, a2).done(function (resData, resDataAB) {
                    var data = resData[0].result
                    if (data && Object.keys(data).length > 0) {
                        markHomeCompany(data, userId_md5, userIdOrLngLat);
                    } else {
                        toastr.warning("查询结果为空(可能diu不正确或者挖掘结果为空)");
                        homeTemp1 = [];
                        companyTemp1 = [];
                    ...
                    var dataAB = resDataAB[0].result;
                    if (dataAB && Object.keys(dataAB).length > 0) {
                        markHomeCompany(dataAB, userId_md5, userIdOrLngLat, true);
                    } else {
                       //toastr.warning("查询结果为空(可能diu不正确或者挖掘结果为空)");
                        $scope.homeDigCompare = $scope.nullHint;
                        homeTemp2 = [];
                        companyTemp2 = [];
                    }

                    $scope.homeDiff    = utils.spherical_distance(homeTemp1, homeTemp2).toFixed(2);
                    $scope.companyDiff = utils.spherical_distance(companyTemp1, companyTemp2).toFixed(2);
                })
            };
