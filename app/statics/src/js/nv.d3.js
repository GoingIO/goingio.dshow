define(['d3'], function () {
    /* nvd3 version 1.8.1 (https://github.com/novus/nvd3) 2015-07-22 */
    'use strict';
    ...
    (function () {
        ...
        nv.models.tooltip = function () {
            ...
            var calcTooltipPosition = function calcTooltipPosition(pos) {
                ...
                nv.dom.read(function () {
                    ...
                    if (hidden) {
                        ...
                        tooltip.interrupt() // cancel running transitions
                            ...
                            .style('-webkit-transform', new_translate).style('-ms-transform', new_translate).style('opacity', 1);
                    }
                });
            };
                ...
                boxEnter.append('rect').attr('class', 'nv-boxplot-box')
                    ...
                    .on('mouseover', function (d, i) {
                        ...
                        dispatch.elementMouseover({
                            key: d.label,
                            value: d.label,
                            series: [{key: 'Q3', value: d.values.Q3, color: d.color || color(d, i)}, {
                                key: 'Q2',
                                value: d.values.Q2,
        ...
        function chart(selection) {
            ...
            selection.each(function (data) {
                ...
                interactiveLayer.dispatch.on('elementMousemove', function (e) {
                    ...
                    }).chartContainer(that.parentNode).valueFormatter(function (d, i) {
        ...
        function chart(selection) {
            selection.each(function (data) {
                ...
                function onBrush() {
                    ...
                    y2Axis.scale(y2)._ticks(nv.utils.calcTicksY(availableHeight1 / 36, data)).tickSize(dataBars.length ? 0 : -availableWidth, 0); // Show the y2 rules only if y1 has none
    ...
    nv.models.linePlusLineChart = function () {
        ...
        function chart(selection) {
            selection.each(function (data) {
                ...
                var wrap = container.selectAll('g.nv-wrap nv-linePlusBar').data([data]);
                var gEnter = wrap.enter().append('g').attr('class', 'nvd3 nv-wrap nv-linePlusLine').append('g');
                gEnter.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
                gEnter.append('g').attr('class', 'nv-legendWrap').datum([{
                    originalKey: 'xx',
                    key: 'yy'
                }]).call(legend);
                gEnter.select('.nv-legendWrap').attr('transform', 'translate(' + (availableWidth - legend.width()) + ', 0)');

                var focusEnter = gEnter.append('g').attr('class', 'ng-focus');
                focusEnter.append('g').attr('class', 'nv-x');
                focusEnter.append('g').attr('class', 'nv-y1');
                focusEnter.append('g').attr('class', 'nv-y2');
                focusEnter.append('g').attr('class', 'nv-linesWrap');
                //chart.update = function () {
                //    container.transition().duration(transitionDuration).call(chart);
                //};
                ...
                //var gEnter = wrap.enter().append('g').attr('class', 'nvd3 nv-wrap nv-linePlusBar').append('g');
                //var g = wrap.select('g');
                var xScale = d3.scale.linear().domain([0, 10000]).range([0, 800]);
                xAxis.scale(xScale);
                xAxis.tickSize(-100, 0);
                focusEnter.select('.nv-x').call(xAxis);
                focusEnter.select('.nv-x').attr('transform', 'translate(0,' + (availableHeight - xAxis.height()) + ')');
            });
        }
    ...
    nv.models.multiBar = function () {
        ...
        function chart(selection) {
            ...
            selection.each(function (data) {
                ...
                y.domain(yDomain || d3.extent(d3.merge(d3.merge(seriesData).map(function (d) {
                        var domain = d.y;
                ...
                var exitTransition = renderWatch.transition(groups.exit().selectAll('g.nv-bar'), 'multibarExit', Math.min(100, duration)).attr('y', function (d, i, j) {
                    var yVal = y0(0) || 0;
                    if (stacked) {
                ...
                var barsEnter = bars.enter().append('g').attr('class', function (d, i) {
                    return getY(d, i) < 0 ? 'nv-bar negative' : 'nv-bar positive';
                ...
                barsEnter.append('rect').attr('height', 0).attr('width', function (d, i, j) {
                ...
                if (getYerr(data[0].values[0], 0)) {
                    ...
                    bars.select('polyline').attr('fill', 'none').attr('stroke', function (d, i, j) {
                ...
                if (barColor) {
                    ...
                    bars.select('rect').style('fill', function (d, i, j) {
                ...
                if (stacked) {
                    barSelection.attr('transform', function (d, i, j) {
                        var yVal = 0;
                    ...
                    barSelection.attr('transform', function (d, i) {
                        ...
                        var yVal = getY(d, i) < 0 ? y(0) : y(0) - y(getY(d, i)) < 1 ? y(0) - 1 : y(getY(d, i)) || 0;
                        return 'translate(' + xVal + ',' + yVal + ')';
                    }).select('rect').attr('width', x.rangeBand() / data.length).attr('height', function (d, i) {
                        return Math.max(Math.abs(y(getY(d, i)) - y(0)), 1) || 0;
                    });
                }
    ...
    nv.models.scatter = function () {
        ...
        chart._calls = new function () {
            ...
            this.highlightPoint = function (seriesIndex, pointIndex, isHoverOver) {
                nv.dom.write(function () {
                    container.select(' .nv-series-' + seriesIndex + ' .nv-point-' + pointIndex).classed('hover', isHoverOver);
                });
            };
        }();
            ...
            selection.each(function (data) {
                ...
                interactiveLayer.dispatch.on('elementMousemove', function (e) {
                    ...
                    interactiveLayer.tooltip.position({
                        left: pointXLocation + margin.left,
                        top: e.mouseY + margin.top
                    }).chartContainer(that.parentNode).valueFormatter(valueFormatter).data({
                        value: xValue,
                        series: allData
                    })();
