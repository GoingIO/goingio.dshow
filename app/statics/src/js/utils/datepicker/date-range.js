...
define(['dateutil'], function(DateUtil){
	...
	function pickerDateRange(inputId, options) {
	    ...
	    var defaults = {
			...
			markCertainDate: {}, // 在某些日期上打上标记，如:{7:[1,3],8:[5,6]}标记8月1号，3号, 9月5号，6号. by hongfei
			markedAllDisabled: false, // 全部不可选，结合markCertainDate使用：只让标记的日期可用
			marked : 'dateRangeMarked' // 标记的样式 by hongfei
	    };
	    //将对象赋给__method变量
	    var __method = this;
	    ...
	    $('#' + this.compareCheckboxId).bind('click', function() {
			...
			$('#' + __method.inputCompareId).css('display', checked ? '' : 'none');
	        // 隐藏对比时间选择
	        $('#' + __method.dateRangeCompareDiv).css('display', checked ? '' : 'none');
			$('#' + __method.compareInputDiv).css('display', checked ? '' : 'none');
	        // 把两个对比时间框置为不可用
	        $('#' + __method.startCompareDateId).css('disabled', checked ? false : true);
	        $('#' + __method.endCompareDateId).css('disabled', checked ? false : true);
	        // 修改表单的 checkbox 状态
	...
	pickerDateRange.prototype.fillDate = function(year, month, index) {
	    ...
	    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
	        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
				...
				if (this.mOpts.markedAllDisabled) {
					tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
					deviation = '4';
				}
				// 标记日期
				if (this.mOpts.markCertainDate[month] && this.mOpts.markCertainDate[month].length > 0) {
					var isMarked = false;

					for (var p in this.mOpts.markCertainDate[month]) {
						if(!isNaN(this.mOpts.markCertainDate[month][p]) || isNaN(parseInt(this.mOpts.markCertainDate[month][p]))){
							if ( this.mOpts.markCertainDate[month][0] === true ){
								isMarked = !!(d.getDate() !== this.mOpts.markCertainDate[month][p]);
								if ( !isMarked ){
									break;
								}
							} else {
					...
					if ( isMarked ){
						tdClass = this.mOpts.theme + '_' + this.mOpts.marked;
						deviation = '0';
					}
