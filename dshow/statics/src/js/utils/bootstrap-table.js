...
//        classes: 'table table-hover',
        ...
        columns: [],
        data: [],
        getData: function(data) {return data},
    ...
    BootstrapTable.LOCALES['zh-CN'] = {
        ...
        formatRecordsPerPage: function (pageNumber) {
            return '每页显示：' + pageNumber ;
        },
        formatShowingRows: function (totalRows, pageNumber) {
            return totalRows + '条记录/共' + pageNumber + '页';
        },
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            style += sprintf('width: %s; ', column.checkbox || column.radio ? '36px' : column.width);
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        // showColumns, showToggle, showRefresh, showExport
        ...
        if (this.options.showExport) {
            html.push(sprintf('<button class="btn btn-default" type="button" name="export" data-container="body" data-toggle="popover" data-placement="left" data-content="%s">', this.options.formatExport()), 
        ...
        if (this.options.showExport) {
            this.$toolbar.find('button[name="export"]')
                .off('click').on('click', this.options.onClickExport);
        }
        ...
        if (this.options.search) {
            html = [];
            html.push(
                '<div class="pull-' + this.options.searchAlign + ' search">', '<span style="margin-right:5px;margin-top:3px;">搜索：</span>',  sprintf('<input class="form-control" style="display: inline;width: auto" type="text" placeholder="%s">',
                        this.options.searchText),
                '</div>');

            this.$toolbar.append(html.join(''));
            $search = this.$toolbar.find('.search input');
            $search.val(this.options.searchDefault);
            that.searchText = this.options.searchDefault;
        ...
        if (typeof this.options.toolbar === 'string') {
            $('<div class="bars pull-left" style="margin-left: -1px;margin-top: 9px;"></div>')
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        html.push(
            '<div class="pull-left pagination-detail">',
                '<span class="pagination-info">',
                    this.options.formatShowingRows(this.options.totalRows, this.totalPages),
                '</span>');
        ...
        $.each(pageList, function (i, page) {
            if (!that.options.smartDisplay || that.options.totalRows >= page || i === 0) {
                ...
                pageNumber.push(sprintf('<span><a href="javascript:void(0)" %s>%s</a></span>', active,  page));
            }
        });
        pageNumber.push('</span>');
        ...
        html.push('<li class="page-number active disabled">',
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    text = ['<td class="bs-checkbox" data-field="'+ field +'">',
                        '<input' +
                            sprintf(' data-index="%s"', i) +
                    ...
                    text = that.options.cardView ?
                        ['<div class="card-view" data-field="'+ field +'">',
                            that.options.showHeader ? sprintf('<span class="title" %s>%s</span>', style,
                                getPropertyFromOther(that.options.columns, 'field', 'title', field)) : '',
                            sprintf('<span class="value">%s</span>', value),
                            '</div>'].join('') :
                        [sprintf('<td data-field="%s" %s %s %s>', field, id_, class_, style),
                            value,
                            '</td>'].join('');
    ...
    BootstrapTable.prototype.initServer = function (silent, queryparams) {
        ...
        that.options.getData(data, function(res) {
    ...
    BootstrapTable.prototype.getCaretHtml = function () {
        return ['<span class="order pull-right">',
                '<span class="frontend-table-'+ (this.options.sortOrder === 'desc' ? 'down' : 'up') +'"></span>',
            '</span>'].join('');
